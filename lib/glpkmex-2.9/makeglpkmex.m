...
disp('Version 2.9 compatible with GLPK 4.36 (or higher)');
disp('(C) 2001-2007, Nicolo'' Giorgetti.');
disp('Maintained by  Niels Klitogrd');
disp('Last updated: Feb 2010.');
...
while 1
   ...
   if isempty(d)
      if usegraph
          if  ispc 
            uiwait(msgbox('Specified directory does not contain GLPK glpk.lib library!','Lib file','error'));
            [filename, pathname] = uigetfile('glpk.lib','Choose glpk.lib file');
            library=[pathname,filename];
          elseif ismac
            uiwait(msgbox('Specified directory does not contain GLPK glpk.dyld library!','Lib file','error'));
            [filename, pathname] = uigetfile('glpk.dyld','Choose glpk.dyld file');
            library=[pathname,filename];
          else
         ...
         fprintf('\t Warning: Specified directory does not contain GLPK libglpk.a (glpk.lib on pc, glpk.dyld on mac) library!\n');
...
if ispc
    if usegraph   
        reply = questdlg( ...
            'Do you compile GLPKMEX with CYGWIN?',...
            'Cygwin on windows', ...
            'Yes','No','No');
    else     
         reply = input('Do you compile GLPKMEX with CYGWIN? Y/N [N]: ', 's');
    end
    
    if ( isempty(reply) || strcmp(reply,'N') || strcmp(reply,'No') || strcmp(reply,'n') || strcmp(reply,'no') )
       % do nothing
    else
