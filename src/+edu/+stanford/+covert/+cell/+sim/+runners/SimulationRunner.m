...
classdef SimulationRunner < handle
    ...
    properties (SetAccess = protected)
        outDir            = ''       %simulation output directory
        logToDisk         = false    %true/false whether or not to log simulation to disk
        logToDb           = false    %true/false whether or not to log simulation to disk
        childDirs         = {}       %2x1 cell array which output directories of initial conditions for child simulation; if empty initial conditions will not be saved for children
        
        useCachedKb       = true     %true/false whether or not to use the cached version of the knowledge base
        useCachedSim      = true     %true/false whether or not to use the cached version of the fitted simulation
        cacheKb           = false    %true/false whether or not to use the cache the knowledge base
        cacheSimulation   = false    %true/false whether or not to use the cache the fitted simulation
        
        kbCache           = 'data/knowledgeBase.mat'        %location of knowledge base cache
        simCache          = 'data/Simulation_fitted%s.mat'  %location of fitted simulation cache
        simJsonCache      = 'data/%s.json'                  %location of JSON-formatted simulation options and parameters cache
        
        lengthSec         = NaN
    end
    
    properties
        seed              = round(1e6 * mod(now(), 1e3));  %simulation seed
        initialConditions = struct()             %initial conditions override; leave empty to use default initial conditions calculated by the Simulation class.
    end
    ...
    methods
        function this = SimulationRunner(varargin)
        ...
        function [sim, kb] = constructKbAndSimulation(this)
            ...
            %% modify network
            if isequal('edu.stanford.covert.cell.sim.runners.SimulationRunner', class(this))
                sim = defSim;
            else
                %% modify structure
        ...
        function runSimulation(this, sim)
            ...
            try
                ...
                sim.applyOptions('seed', this.seed);
                if ~isempty(this.lengthSec) && ~isnan(this.lengthSec)
                    sim.applyOptions('lengthSec', this.lengthSec);
                end                
                sim.run(ic, loggers);
